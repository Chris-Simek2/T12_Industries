...
const int right_LED_PIN = 18; // LED PIN: Arduino Uno = 13, Adafruit RP2040 = GPIO1
const int left_SPEAKER_PIN = 0; // SPEAKER PIN: Arduino Uno = 12, Adafruit RP2040 = GPIO0
const int left_LED_PIN = 1;
const int right_SPEAKER_PIN = 20;
...
*/
void center_BSD_notification(int currVal, int prevVal1, int prevVal2, int prevVal3) {
  if (blindSpotDetection(currVal, prevVal1, prevVal2, prevVal3) == true) {
    digitalWrite(left_LED_PIN, HIGH);
    digitalWrite(right_LED_PIN, HIGH);
    tone(left_SPEAKER_PIN, 100);
    tone(right_SPEAKER_PIN, 100);
  } else {
    digitalWrite(left_LED_PIN, LOW);
    digitalWrite(right_LED_PIN, LOW);
    noTone(left_SPEAKER_PIN);
    noTone(right_SPEAKER_PIN);
  }
}

void right_BSD_notification(int currVal, int prevVal1, int prevVal2, int prevVal3) {
  if (blindSpotDetection(currVal, prevVal1, prevVal2, prevVal3) == true) {
    digitalWrite(right_LED_PIN, HIGH);
    tone(right_SPEAKER_PIN, 100);
  } else {
...
void left_BSD_notification(int currVal, int prevVal1, int prevVal2, int prevVal3) {
  if (blindSpotDetection(currVal, prevVal1, prevVal2, prevVal3) == true) {
    digitalWrite(left_LED_PIN, HIGH);
    tone(left_SPEAKER_PIN, 100);
  } else {
...
//*****************************************//
 ...
 // Adding left ultrasonic sensor values into matrix
  sonarDistance(trigPin1, echoPin1);
  for (int i = 3; i > 0; i--){
    left_Matrix[i][0] = left_Matrix[i-1][0];
  }
  delay(10);
  left_Matrix[0][0] = distance;
  blindSpotDetection(left_Matrix[0][0], left_Matrix[1][0], left_Matrix[2][0], left_Matrix[3][0]);
  left_BSD_notification(left_Matrix[0][0], left_Matrix[1][0], left_Matrix[2][0], left_Matrix[3][0]);

  // Adding ML ultrasonic sensor values into matrix
  sonarDistance(trigPin2, echoPin2);
  for (int i = 3; i > 0; i--){
    ML_Matrix[i][0] = ML_Matrix[i-1][0];
  }
  delay(10);
  ML_Matrix[0][0] = distance;
  // blindSpotDetection(ML_Matrix[0][0], ML_Matrix[1][0], ML_Matrix[2][0], ML_Matrix[3][0]);
  // left_BSD_notification(ML_Matrix[0][0], ML_Matrix[1][0], ML_Matrix[2][0], ML_Matrix[3][0]);

  // Adding middle ultrasonic sensor values into matrix
  ...
  center_BSD_notification(middle_Matrix[0][0], middle_Matrix[1][0], middle_Matrix[2][0], middle_Matrix[3][0]);
  
  // Adding MR ultrasonic sensor values into matrix
  sonarDistance(trigPin4, echoPin4);
  for (int i = 3; i > 0; i--){
    MR_Matrix[i][0] = MR_Matrix[i-1][0];
  }
  delay(10);
  MR_Matrix[0][0] = distance;
  // blindSpotDetection(MR_Matrix[0][0], MR_Matrix[1][0], MR_Matrix[2][0], MR_Matrix[3][0]);
  // right_BSD_notification(MR_Matrix[0][0], MR_Matrix[1][0], MR_Matrix[2][0], MR_Matrix[3][0]);

  sonarDistance(trigPin5, echoPin5);
  for (int i = 3; i > 0; i--){
    right_Matrix[i][0] = right_Matrix[i-1][0];
  }
  delay(10);
  right_Matrix[0][0] = distance;
  blindSpotDetection(right_Matrix[0][0], right_Matrix[1][0], right_Matrix[2][0], right_Matrix[3][0]);
  right_BSD_notification(right_Matrix[0][0], right_Matrix[1][0], right_Matrix[2][0], right_Matrix[3][0]);
